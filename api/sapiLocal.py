...
# Dependency Packages
try: from comtypes.client import CreateObject
except ImportError: raise ImportError("Missing required dependency: comtypes (https://pypi.python.org/pypi/comtypes)")
...
class TTSClient(object):
	...
	def _audio_frames(self):
		...
		try: return cStringIO.StringIO(bytearray(self._stream.GetData()))
		finally:
			del self._stream
			self._stream = CreateObject("SAPI.SpMemoryStream")
			self._engine.AudioOutputStream = self._stream
	...
	def _play_frames(self):
		...
		while True:
			...
			if not frames: break
			else: self.audioChunks.put(frames)
	...
	def volume(self, value):
		...
		else: self._engine.Volume = value
