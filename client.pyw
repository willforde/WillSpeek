...
except ImportError: import tkinter as tk # Python3
...
class ClipboardMonitor(object):
		...
		while self.running:
			if self.enabled:
				...
				if prev is None:
					...
					removeText = readMore.findall(data)
					for text in removeText: data = data.replace(text,"")
					
					for part in data.strip().replace("\r\n","\n").split("\n\n"):
	...
	def get_clipboard_text(self):
		try: return self.gui.clipboard_get()
		except Exception:
			time.sleep(.5)
			return False
...
# Testing
from systry import SysTrayIcon

# Initialize Clipboard Processor
t = threading.Thread(target=SysTrayIcon, args=(MenuItems(),))
t.daemon = True
t.start()
